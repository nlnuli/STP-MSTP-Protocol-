...
##建立交换机类
print("-------添加节点中--------")
file_path = '/Users/yewen/Desktop/2023毕业设计/code/switch.xlsx'
    ...
    for i in port_list:
        ...
        for j in target_switch.port:
                ...
                if min > p1:
                    min = p1
                    #更优秀
                    short_path = path(i,j,g)
                    sbid = find_bid(short_path[1],switch_list)
...
for edge in g.edges(data = True):
    ...
    ##找到了
    G.add_edge(pair_name[0], pair_name[1], weight = weight, AP = 'false', color = 'red')
    if flag == 1:
        G[pair_name[0]][pair_name[1]]['AP'] = 'true'
        G[pair_name[0]][pair_name[1]]['color'] = 'blue'
