...
for i in range(data.shape[0]) :
    ...
    g.add_node(port_data[0],PID = pid, Root_bridge_ID = '0',ERPC = 0, \
               Region_bridge_ID = 0, IRPC = 0, Designated_BID = 0, Designated_PID = 0, received_PID = 0,Port_status = 'None',switch_bid = -1,portID = port_data[1],Port_priority = port_data[2],)
##对port 的switch信息进行维修
for switch in switch_list:
    ...
    g.add_node(switch.switch_name,PID = -1, Root_bridge_ID = switch.bid,ERPC = -1, \
               Region_bridge_ID = switch.bid, IRPC = -1, Designated_BID = -1, Designated_PID = -1, received_PID = -1,Port_status = 'switch',switch_bid = switch.bid,portID = -1,Port_priority = -1)
...
for region in region_dict:
    ...
    for switch_name in switch_region_list:
        switch_bid.append(find_bid(switch_name,switch_list))
    region_root_bid = min(switch_bid)
    ##目前可以找到每一个域内对应switch的bid 和 域的名字
    region_root_name = find_name(region_root_bid,switch_list)
    ...
    for switch_name in switch_region_list:
        region_switch_list.append(find_switch(switch_name,switch_list))
    for switch in region_switch_list:
        ...
        if switch.bid == region_root_bid:
            ...
            for port in target_switch.port :
                ...
                g.nodes[switch.switch_name + '-' + str(port)]['Designated_PID'] = g.nodes[switch.switch_name + '-' + str(port)]['PID']
        ...
        for port in port_list:
            ...
            g.nodes[port_name]['Designated_BID'] = find_designated_bridge(Designated_port_name, switch_list)
            g.nodes[port_name]['Designated_PID'] = find_designated_pid(Designated_port_name,g)
            g.nodes[port_name]['received_PID'] = find_self_pid(port_name,g)


        for port in port_list:
            if switch.switch_name + '-' + str(port) in backup_name:
                continue
            g.add_edge(switch.switch_name, switch.switch_name + '-' + str(port), cost=0, VLAN=-1,region_name=switch.region_name)
...
recover_backup_edge(backup_del,g)
recover_backup_node(backup,g)
